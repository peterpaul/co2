...
O_IMPLEMENT(ObjectTypeDeclaration, void *, ctor, (void *_self, va_list *app))
{
  struct ObjectTypeDeclaration * self = O_CAST(_self, ObjectTypeDeclaration());
...
void ObjectTypeDeclaration_generate_constructor_definition(void *_constructor_decl, va_list * app)
{
  ...
  struct ObjectTypeDeclaration * class_decl = O_CAST(va_arg(*app, struct ObjectTypeDeclaration *), ObjectTypeDeclaration());
...
void ObjectTypeDeclaration_generate_method_implementation_2(void *_interface_name, va_list *app)
{
  ...
  struct Token * implementation_name = O_CAST(va_arg(*app, struct Token *), Token());
  struct Declaration * _decl = O_CALL(global_scope, lookup_in_this_scope, interface_name);
  struct InterfaceDeclaration * interface_decl = O_CAST(_decl, InterfaceDeclaration());

  fprintf(out, "O_OBJECT_IF(");
  O_CALL(interface_name, generate);
  fprintf(out, ");\n");
  O_CALL(interface_decl->members, map_args, ObjectTypeDeclaration_generate_interface_method_registration, implementation_name);
...
void ObjectTypeDeclaration_generate_constructor_implementation(void *_constructor_decl, va_list *app)
{
  ...
  O_CALL(class_decl->name, generate);
  fprintf(out, "());\n");
  O_CALL(constructor_decl->formal_arguments, map, ObjectTypeDeclaration_generate_constructor_arguments);
