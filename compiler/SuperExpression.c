...
O_IMPLEMENT(SuperExpression, void *, ctor, (void *_self, va_list *app))
{
  ...
  self->actual_arguments = O_BRANCH_CAST(va_arg(*app, struct RefList *), RefList());
...
O_IMPLEMENT(SuperExpression, void, type_check, (void *_self))
{
  ...
  /* get method context */
  self->method_context = O_CALL(current_context, find, ConstructorDeclaration());;
  if (self->method_context == NULL) {
    self->method_context = O_CALL(current_context, find, FunctionDeclaration());
  }

  struct ClassDeclaration * super = (struct ClassDeclaration *) O_CALL(global_scope, lookup, self->class_context->superclass);
  ...
  if (o_is_of(self->method_context, ConstructorDeclaration()))
    {
      ...
      if (ctor_name == NULL)
	{
	  ctor_name = O_CALL_CLASS(Token(), new, "ctor", IDENTIFIER, filename, self->super->line);
	}
      O_CALL(ctor_name, retain);
      struct ConstructorDeclaration * ctor_decl = (struct ConstructorDeclaration *) O_CALL(super->member_scope, lookup, ctor_name);
    ...
    {
      struct FunctionDeclaration * function_decl = (struct FunctionDeclaration *) O_CALL(super->member_scope, lookup, self->method_context->name);
	...
	{
	  struct ArgumentDeclaration * arg_decl = O_CALL(function_decl->formal_arguments, get, i);
	  struct Expression * arg_expr = O_CALL(self->actual_arguments, get, i);
	  O_CALL(arg_expr, type_check);
	  O_CALL(arg_decl->type, assert_compatible, arg_expr->type);
	}
